...
import { type CollectionItem, type PriceEstimate } from "@poke-organizer/shared";
  ...
  return (
    <article
      ...
      className={`collection-item-card collection-item-card--${kind} group relative cursor-pointer rounded-[22px] border bg-white/72 p-2 shadow-sm transition duration-200 hover:-translate-y-1 hover:shadow-soft ${
        selected ? "border-brand/50 bg-brand/10 shadow-soft" : "border-line/80 bg-white/70"
      }`}
    >
      {onToggleSelection && (
        <button
          ...
          aria-label={selected ? "Remover da atualizacao de valores" : "Selecionar para atualizar valores"}
        >
      ...
      {item.quantity > 1 && (
        <span className={`absolute right-4 z-30 rounded-full border border-white/85 bg-night/88 px-2.5 py-1 text-xs font-black text-white shadow-sm backdrop-blur ${onToggleSelection ? "top-14" : "top-4"}`}>
          x{item.quantity}
        </span>
      )}
      ...
      <div className="grid min-h-[58px] gap-1 px-1 py-2">
        <p className="truncate text-sm font-black text-ink">{priceText}</p>
        <p className="truncate text-xs font-semibold text-slate-500">{item.card.name}</p>
      </div>
